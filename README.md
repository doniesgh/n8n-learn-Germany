...
This usually indicates that the parameters generated by the AI do not match the tool's defined schema.
...
If the translation API returns something similar to:

```text
NO QUERY SPECIFIED
```

check that the request contains the expected translation parameter.
...
must contain the text to translate.

Also verify that the HTTP Request node uses the correct method and body format.

---

## "Tool call validation failed"

Check the AI tool schema.

For example, if the tool expects:
...
do not allow the AI to send additional parameters such as:
...
unless these are explicitly part of the tool schema.
...
This README provides the high-level documentation of the project.

Additional documentation can be maintained for:
